...
export function HomeInquiryForm() {
  ...
  const onSubmit = async (e) => {
    ...
    //   toast.success("Got it — our counsellor will call you shortly.");
    //   setForm(initialForm);
    // } catch {
  ...
  <div className="relative rounded-3xl bg-primary p-8 text-primary-foreground md:p-14 mx-auto max-w-5xl shadow-lg shadow-primary">
        ...
        <div className="relative grid gap-10 md:grid-cols-[1fr_1.1fr]">
          <div>
            <p className="text-xs uppercase tracking-wider text-gold">Talk to an advisor</p>
            <h2 className="mt-2 font-display text-3xl md:text-4xl">Get a personal shortlist — free.</h2>
            <p className="mt-4 text-sm leading-relaxed text-primary-foreground/85">
              Tell us a little about your child. Our counsellor will call within 24 hours with 3
              boarding schools that genuinely suit your family. No fees, no pressure.
            </p>
            <ul className="mt-6 space-y-3 text-sm text-primary-foreground/90">
              <li className="flex items-start gap-2">
                <GraduationCap size={16} className="mt-0.5 text-gold" /> Matched to your child's
                age, board &amp; interests
              </li>
              <li className="flex items-start gap-2">
                <ShieldCheck size={16} className="mt-0.5 text-gold" /> Your data is never sold or
                shared
              </li>
              <li className="flex items-start gap-2">
                <HeartHandshake size={16} className="mt-0.5 text-gold" /> Talk to a real human,
                not a bot
              </li>
            </ul>
          </div>
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="text-xs">
                <span className="text-muted-foreground">Child's current grade</span>
                <input
              ...
              <label className="text-xs">
                <span className="text-muted-foreground">Preferred city / state</span>
                <input
              ...
              <label className="text-xs sm:col-span-2">
                <span className="text-muted-foreground">Anything else we should know?</span>
                <textarea
                  rows={3}
                  value={form.message}
